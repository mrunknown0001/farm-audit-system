	...
	<section class="content">
		...
		<div class="row">
			<div class="col-md-6 col-md-offset-3">
				<form id="SubLocationForm" action="{{ route('sub.location.post.add') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
					@csrf
					<div id="LocationNameDiv" class="form-group {{ $errors->first('location_name') ? 'has-error' : ''  }}">
						...
						<select name="location_name" id="location_name" class="form-control" required>
							...
							@foreach($locations as $key => $l)
								<option value="{{ $l->id }}">{{ $l->farm->code . '-' . $l->location_name }}</option>
							@endforeach
						</select>
            ...
            @endif
					...
					<div id="SubLocationNameDiv" class="form-group {{ $errors->first('sub_location_name') ? 'has-error' : ''  }}">
						...
						<input type="text" name="sub_location_name" id="sub_location_name" placeholder="Sub Location Name" class="form-control" required>
            ...
            @endif
					...
					<div id="SubLocationCodeDiv" class="form-group {{ $errors->first('sub_location_code') ? 'has-error' : ''  }}">
						...
						<input type="text" name="sub_location_code" id="sub_location_code" placeholder="Sub Location Code" class="form-control" required>
